...
| `office/v1/` | `OfficeDataV1` |
| `payment/v1/` | `PaymentDetailDataV1`, `PaymentTypeDataV1` |
| `portfolio/v1/` | `ChargeDataV1`, `PortfolioAccountDataV1`, `RateDataV1` |
| `share/v1/` | `ShareAccountDataV1`, `ShareProductDataV1`, `ShareAccountTransactionDataV1` |
| `workingcapitalloan/v1/` | `WorkingCapitalLoanTransactionDataV1` |
| `generic/v1/` | `CommandProcessingResultV1`, `CurrencyDataV1`, `EnumOptionDataV1`, `CalendarDataV1`, `CodeValueDataV1`, `StringEnumOptionDataV1` |
...
fineract-core/src/main/java/org/apache/fineract/infrastructure/event/
  external/service/serialization/serializer/
    BusinessEventSerializer.java      # interface
    AbstractBusinessEventWithCustomDataSerializer.java  # optional base class
    BusinessEventSerializerFactory.java
...
Each serializer implements the `BusinessEventSerializer` interface and:
1. Declares which business event types it can handle via `canSerialize(event)`.
2. Calls the mapper to get the Avro payload via `toAvroDTO(event)`.
3. Declares the target Avro schema class via `getSupportedSchema()`.
4. The `ExternalEventService` wraps the payload in the `MessageV1` envelope and persists the serialized bytes.
