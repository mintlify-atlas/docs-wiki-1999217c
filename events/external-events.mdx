...
**External events** are the durable, transport-ready representation. `ExternalEventService` (package `org.apache.fineract.infrastructure.event.external.service`) converts a business event into an `ExternalEvent` JPA entity and persists it. A background publisher thread then reads those rows and hands them off to whichever `ExternalEventProducer` is configured.
...
<Tabs>
  <Tab title="Loan events">
    Events in package `org.apache.fineract.infrastructure.event.business.domain.loan` (fineract-loan module):
