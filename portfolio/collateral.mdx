...
The legacy system lives in `org.apache.fineract.portfolio.collateral` (the `LoanCollateral` entity is in `fineract-loan`; the API and service layers are in `fineract-provider`) and attaches collateral directly to a loan. It is a lightweight record consisting of:
