...
├── listener/     # HookEventListener (dispatches after write commands)
├── mapper/       # HookMapper
└── processor/    # WebHookProcessor, JmsHookProcessor
```
...
sequenceDiagram
    ...
    participant LISTENER as HookEventListener
    participant SVC as HookWritePlatformService
    participant PROC as WebHookProcessor

    CMD->>CMD: execute write command
    CMD->>LISTENER: publishEvent(HookEvent)
    LISTENER->>SVC: processHooks(entityName, actionName, payload)
    SVC->>SVC: findHooks(entityName, actionName)
    SVC->>PROC: process(hook, payload)
    PROC->>PROC: HTTP POST to Payload URL
```

The `HookEventListener` receives a Spring `ApplicationEvent` after each write command completes. It looks up all hooks registered for the fired entity+action pair and dispatches each one asynchronously via the appropriate `HookProcessor`.
