...
<include file="tenant/changelog-tenant.xml"
         context="tenant_db AND !initial_switch"/>
<!-- module sub-changelogs: loan, investor, savings, progressiveloan, ... -->
...
Custom deployments can add changelogs under `db/custom-changelog/` (enabled via the `custom_changelog` context):
