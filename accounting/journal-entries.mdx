...
`GLClosure` (table: `acc_gl_closure`) prevents backdated entries into completed accounting periods.
...
The `TrialBalance` entity (table: `m_trial_balance`) stores pre-computed debit and credit totals per GL account per period. It is populated by the `UpdateTrialBalanceDetailsTasklet` Spring Batch job in `org.apache.fineract.accounting.glaccount.jobs.updatetrialbalancedetails`.
