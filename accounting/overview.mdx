...
`GLClosure` (table: `acc_gl_closure`) prevents journal entries from being backdated into a closed accounting period. `GLClosuresApiResource` at `/api/v1/glclosures` manages the create/update/delete lifecycle of closures per office.
...
The trial balance is maintained by the `UpdateTrialBalanceDetailsTasklet` (Spring Batch job in `org.apache.fineract.accounting.glaccount.jobs.updatetrialbalancedetails`). It aggregates journal entry debits and credits per GL account and stores summary rows in the `m_trial_balance` table (`TrialBalance` entity).
