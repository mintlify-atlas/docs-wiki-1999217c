...
2. **Command source persistence** — inserts a `CommandSource` record in `m_maker_checker_audit` with status `UNDER_PROCESSING`.
...
sequenceDiagram
    ...
    participant Audit as m_maker_checker_audit
...
- A **checker** (different user with checker permission) calls `approveEntry(commandSourceId)` on `PortfolioCommandSourceWritePlatformService`, which re-executes the persisted `CommandAsJson` with `isApprovedByChecker = true`, bypassing the rollback.
