...
Beyond the primary status, `SavingsAccount` tracks a `sub_status_enum` column corresponding to `SavingsAccountSubStatusEnum` (values include `NONE`, `INACTIVE`, `DORMANT`). An account that becomes dormant due to inactivity has its sub-status updated; posting a transaction resets it back to `NONE`.
