...
<Accordion title="LOAN_DELINQUENCY_CLASSIFICATION — SetLoanDelinquencyTagsBusinessStep">
...
```
Enum name: LOAN_DELINQUENCY_CLASSIFICATION
...
```
Enum name: CHECK_DUE_INSTALLMENTS
Class: org.apache.fineract.cob.loan.CheckDueInstallmentsBusinessStep
```
</Accordion>

<Accordion title="BUY_DOWN_FEE_AMORTIZATION — BuyDownFeeAmortizationBusinessStep">
Amortizes buy-down fee amounts for applicable loan products over the loan term.

```
Enum name: BUY_DOWN_FEE_AMORTIZATION
Class: org.apache.fineract.cob.loan.BuyDownFeeAmortizationBusinessStep
```
</Accordion>

<Accordion title="CAPITALIZED_INCOME_AMORTIZATION — CapitalizedIncomeAmortizationBusinessStep">
Amortizes capitalized income (e.g., capitalised interest or fees) on progressive-model loans.

```
Enum name: CAPITALIZED_INCOME_AMORTIZATION
Class: org.apache.fineract.cob.loan.CapitalizedIncomeAmortizationBusinessStep
```
</Accordion>

## Locking Mechanism

Fineract uses a database-backed locking table (`m_loan_account_locks`) to prevent API requests from modifying loans while COB is processing them. The lock owner is tracked by the `LockOwner` enum.

```mermaid
sequenceDiagram
    participant W as Worker (ApplyLoanLockTasklet via CobWorkerStepListener)
    participant DB as m_loan_account_locks
    participant R as Worker (LoanItemReader)
    participant A as API Request

    W->>DB: INSERT lock rows for partition's loan IDs (LOAN_COB_CHUNK_PROCESSING)
    R->>DB: Read loan IDs; verify locks held
    A->>DB: Attempt operation on locked loan
    DB-->>A: 423 Locked (LoanCOBApiFilter intercepts)
    R->>DB: Process loan, complete
    R->>DB: DELETE lock rows for processed loans
```
...
| `LoanCOBApiFilter` | `org.apache.fineract.infrastructure.jobs.filter` | HTTP filter that returns 423 for API requests targeting locked loans (extends abstract `COBApiFilter`) |
...
`UnlockProcessedLoansTasklet` calls `AccountLockService.removeOrphanedLocksForProcessedAccounts()` to clean up any lock records that remain for loans whose COB processing completed successfully.
