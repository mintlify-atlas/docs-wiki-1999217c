...
<CardGroup cols={2}>
  ...
  <Card title="SchedulerDetail" icon="gear">
    Singleton entity (table `scheduler_detail`) that stores global scheduler state: `suspended` flag (whether the scheduler is in standby), `executeInstructionForMisfiredJobs`, and `resetSchedulerOnBootup`.
    <br/>Package: `org.apache.fineract.infrastructure.jobs.domain`
  </Card>
