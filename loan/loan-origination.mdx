...
When set to `true`, all `@ConditionalOnProperty`-annotated beans are registered. When `false` (the default), the core `fineract-loan` module uses `LoanOriginatorLinkingServiceNoOp` — a no-operation implementation of `LoanOriginatorLinkingService` — so the loan creation flow is completely unaffected.
