...
<Accordion title="Full LoanEvent list">
  ...
  | `LOAN_RECOVERY_PAYMENT` | Post-write-off recovery payment posted |
  ...
  | `LOAN_CHARGE_PAYMENT` | Payment applied to a specific charge |
  | `LOAN_CLOSED` | Generic close event |
  | `LOAN_EDIT_MULTI_DISBURSE_DATE` | Multi-disbursement tranche date edited |
  | `LOAN_REFUND` | Refund posted against the loan |
  | `LOAN_ADJUST_TRANSACTION` | Existing transaction adjusted or reversed |
  ...
  | `INTERST_REBATE_OWED` | Interest rebate owed (legacy; note: enum name is intentionally misspelled in source) |
</Accordion>
