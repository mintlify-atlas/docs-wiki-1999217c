...
<Note>
  `AppUser`, `Role`, and `Permission` live in the `fineract-core` module, not `fineract-provider`. The domain classes are shared across modules via the core artifact.
</Note>

`AppUser` extends `AbstractPersistableCustom<Long>` and implements `PlatformUser` (which itself extends Spring Security's `UserDetails`). The critical security-relevant fields are:
...
// All four delegates share this implementation:
// hasNotPermissionForAnyOf("ALL_FUNCTIONS", "ALL_FUNCTIONS_READ", matchPermission)
// This means ALL_FUNCTIONS_READ satisfies every prefix — READ, CREATE, UPDATE, DELETE.
...
<Note>
  The private `validateHasPermission` helper always includes `"ALL_FUNCTIONS_READ"` in its authority check regardless of the action prefix. In practice this means a user with `ALL_FUNCTIONS_READ` can pass the service-layer `validateHasCreatePermission` / `validateHasUpdatePermission` / `validateHasDeletePermission` checks. The URL-level `SecurityConfig` matchers use `ALL_FUNCTIONS_WRITE` for write endpoints, providing the effective enforcement boundary.
</Note>
