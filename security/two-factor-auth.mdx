...
Apache Fineract's two-factor authentication (2FA) feature adds a second verification step on top of HTTP Basic Auth, requiring users to prove possession of a registered delivery channel (email or SMS) in addition to their password. The feature is implemented entirely within the `fineract-security` module and is fully optional — it activates only when `fineract.security.2fa.enabled=true`. When active, every protected API endpoint requires the `Fineract-Platform-TFA-Token` request header in addition to the standard `Authorization: Basic …` header, except for endpoints that only require fully-authenticated access (such as `POST /api/v1/twofactor` itself).
...
<Note>
  2FA is only supported in Basic Auth mode (`fineract.security.basicauth.enabled=true`). When OAuth2 mode is active, the `TwoFactorApiResource` bean is still conditionally registered, but the filter integration is specific to the Basic Auth `SecurityConfig` and `AuthorizationServerConfig` classes.
</Note>
...
3. Otherwise, read the `Fineract-Platform-TFA-Token` header. If present, call `TwoFactorService.fetchAccessTokenForUser()`. If the returned `TFAccessToken` is `null` or `!isValid()`, return HTTP 401.
